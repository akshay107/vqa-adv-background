...
import ops_adv as ops
from config_adv import config
from mac_cell_adv import MACCell
from resnet101_tf import resnet101
'''
...
class MACnet(object):
    ...
    def addPlaceholders(self):
        with tf.variable_scope("Placeholders"):
            ...
            self.imagesPlaceholder = tf.placeholder(tf.float32, shape = (None, 224, 224, 3))
            #self.imagesAll = self.imagesPlaceholder
            self.maskPlaceholder = tf.placeholder(tf.bool, shape = (None, 224, 224, 3))
            self.gradscaling = tf.placeholder(tf.float32, shape=None)
    ...
    def createFeedDict(self, data, images, mask, train):
        feedDict = {
            self.questionsIndicesAll: data["questions"],
            self.questionLengthsAll: data["questionLengths"],
            self.imagesPlaceholder: images,
            self.maskPlaceholder: mask,
            self.gradscaling: data["grad_scaling"],
            self.answersIndicesAll: data["answers"],
            
            self.dropouts["encInput"]: 1.0,
            self.dropouts["encState"]: 1.0,
            self.dropouts["stem"]: 1.0,
            self.dropouts["question"]: 1.0, #_
            self.dropouts["memory"]: 1.0,
            self.dropouts["read"]: 1.0, #_
            self.dropouts["write"]: 1.0,
            self.dropouts["output"]: 1.0,
    ...
    '''
    def runBatch(self, sess, data, images, mask, train, getAtt = False):     
        ...
        time0 = time.time()
        feed = self.createFeedDict(data, images, mask, train) 

        time1 = time.time()
        _, loss, predsInfo, logit, gradNorm, advImg, advImgscaled, image_feature, attentionMaps = sess.run(
            [trainOp, self.lossAll, predsOp, self.logits, gradNormOp, self.advImg, self.adv_img_scaled,self.images, attOp], 
            feed_dict = feed)
        
        time2 = time.time()  
        #print("predsInfo is", predsInfo)
        ...
        return {"loss": loss,
                ...
                "acc": predsInfo[2],
                "attmaps": attentionMaps, 
                "logits": logit,
                "advImg": advImg,
                "advImgScaled": advImgscaled,
            ...
            for i in range(config.gpusNum):
                with tf.device("/gpu:{}".format(i)):
                    with tf.name_scope("tower{}".format(i)) as scope:
                        ...
                        resnet = resnet101(self.imagesPlaceholder,self.maskPlaceholder,self.gradscaling)
                        self.images = resnet.imagefeatures
                        self.advImg = resnet.adv_input
                        self.adv_img_scaled = resnet.adv_img_scaled
