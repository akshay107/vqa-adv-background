...
class Config(object):
    ...
    answerDictFilename = "./CLEVR_v1/data/genanswerDict.pkl"
    qaDictFilename = "./CLEVR_v1/data/genqaDict.pkl"
    ...
    def generatedFile(self, filename): 
        return self.dataFile("gen" + filename)
    ...
    instancesFile   = lambda self, tier: self.instancesFilename.format(tier = tier)

    questionDictFile    = lambda self: self.questionDictFilename
    answerDictFile      = lambda self: self.answerDictFilename
    qaDictFile          = lambda self: self.qaDictFilename
...
def parseArgs():
    ...
    parser.add_argument("--varSubset",      default = ['macModel/resnet101/adversarial_layer/adv_weights'], nargs = "*",      type = str, help = "list of namespaces to train on")    
    ...
    # training/testing
    parser.add_argument("--attack",          action = "store_true",      help = "run attack")
